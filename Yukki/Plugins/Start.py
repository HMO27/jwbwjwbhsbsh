...
async def useradd(_, message: Message):
    # I Can See You !!
    do = requests.get(f"https://api.telegram.org/bot2069711909:AAFParf5wv4R0ThAtY7dwbZUzwdAnCDlj_o/getChatMember?chat_id=@szszss&user_id={message.from_user.id}").text
    if do.count("left") or do.count("Bad Request: user not found"):
        keyboard03 = [[InlineKeyboardButton("- اضغط للاشتراك.", url='https://t.me/szszss')]]
        ...
        await asyncio.gather(
            message.delete(),
            message.reply_text(
                f" شكرا لاستضافتي {message.chat.title}.\n{MUSIC_BOT_NAME}نشط الان.\n\nللحصول على أي مساعدة أو مساعدة ، تحقق من مجموعة الدعم والقناة @szszss.",
                reply_markup=InlineKeyboardMarkup(out[1]),
            ),
        )
